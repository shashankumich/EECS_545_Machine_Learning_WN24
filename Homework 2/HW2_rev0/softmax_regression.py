...
def compute_softmax_probs(X: np.ndarray, W: np.ndarray) -> np.ndarray:
    ...
    arg_max = np.max(arg, axis=1, keepdims=True)
    arg -= arg_max

    exp_arg = np.exp(arg)
    
    dem = np.sum(exp_arg, axis=1, keepdims=True)
...
def gradient_ascent_train(X_train: np.ndarray,
    ...
    for epoch in range(max_iters):
        ...
        prob = compute_softmax_probs(X_train, W)
        ...
        for i in range(num_class):
          ...
          delta_W[i] = np.sum(X_train * (ind - prob[:,i:i+1]), axis=0)
...
def compute_accuracy(X_test: np.ndarray,
    ...
    prob = compute_softmax_probs(X_test, W)
    # print(np.shape(prob))
    y_pred = 1 + np.argmax(prob, axis=1)
