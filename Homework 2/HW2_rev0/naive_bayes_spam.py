...
def train_naive_bayes(X: np.ndarray, Y: np.ndarray,
    ...
    # Computing mu
    N_Jspam = np.sum(X_spam, axis=0)
    N_Jnospam = np.sum(X_nospam, axis=0)


    mu_spam = (N_Jspam + 1)/(np.sum(N_Jspam)+vocab_size)
    mu_non_spam = (N_Jnospam + 1)/(np.sum(N_Jnospam)+vocab_size)
...
def test_naive_bayes(X: np.ndarray,
    ...
    # Based on Hint1: Computing Logs
    logPhi = np.log(phi)
    logNonPhi = np.log(1-phi)
    logMuSpam = np.log(mu_spam)
    logNonMuSpam = np.log(1-mu_spam)
    logMuNonSpam = np.log(mu_non_spam)
    logNonMuNonSpam = np.log(1-mu_non_spam)

    # Based on Hint2: For each email
    num_mails, vocab_size = X.shape
    for i in range(num_mails):
        Tlogspam = X[i,:]*logMuSpam
        # Flogspam = (1 - X[i,:])*logNonMuSpam
        log_prob_spam = np.sum(Tlogspam) +  logPhi
        
        TlogNonspam = X[i,:]*logMuNonSpam
        # FlogNonspam = (1 - X[i,:])*logNonMuNonSpam
        log_prob_Non_spam = np.sum(TlogNonspam) +  logNonPhi

        if log_prob_spam > log_prob_Non_spam:
          pred[i] = 1
...
def get_indicative_tokens(mu_spam: np.ndarray,
    ...
    buf = np.log(mu_spam/mu_non_spam)
    arg_buf = np.argsort(buf)
    idx_list = arg_buf[-top_k:]
    # raise NotImplementedError("TODO: Add your implementation here.")
    ###################################################################
    #                        END OF YOUR CODE                         #
    ###################################################################
