...
def naive_logistic_regression(X: np.ndarray, Y: np.ndarray, max_iters = 100) -> np.ndarray:
    ...
    for iter in range(max_iters):
        ...
        for data_x, data_y in zip(X, Y):
            ...
            # Computing gradient 
            grad = grad + (data_y - sigmoid(data_x @ w)) * data_x
            # print((grad))
            # grad[iter] = np.append(grad, grad_buf, axis=1)

            # Computing Hessian 
            H = H - (sigmoid(data_x @ w) * (1 - sigmoid(data_x @ w))) * np.outer(data_x, data_x)
            # print(data_x @ data_x)
            
            # H = np.append(H, H_buf, axis=1)
...
def vectorized_logistic_regression(X: np.ndarray, Y: np.ndarray, max_iters = 100) -> np.ndarray:
  ...
  """
    ...
    for iter in range(max_iters):
        ...
        # Compute the vectorized gradient = X^{T} (p(X) - Y)
        grad = np.transpose(X) @ (sigmoid(X @ w) - Y)
        
        # Compute the vectorized hessian = X^{T} @ diag(p(X) * (1 - p(X))) @ X 
        H = np.transpose(X) @ np.diag( sigmoid(X @ w) * (1 - sigmoid(X @ w))) @ X
