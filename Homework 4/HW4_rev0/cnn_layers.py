...
def conv_forward(x, w):
    ...
    for j in range(0, H_prime):                                             #
        for i in range(0, W_prime):                                         #
            ...
            image_patch = x[:,:,j:j+HH,i:i+WW]
            image_patch = image_patch.reshape(N,-1)
            filter = w.reshape(F,-1).T
            # print(np.shape(image_patch))    
            # print(np.shape(filter))    
            # k                                  
                                                                   #
            out[:,:,j,i] =  np.dot(image_patch,filter)

    # raise NotImplementedError("TODO: Add your implementation here.")
    # print(N,F,C,H,W,HH,WW)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w)
...
def conv_backward(dout, cache):
    ...
    ###########################################################################
    # Implementing the dl/dkf,c - similar to conv_forward
    # print(N,F,C,H,W,HH,WW,H_prime,W_prime,pad_len)
    # for j in range(0, HH):                                             #
    #     for i in range(0, WW):                                         #
            
    #         image_patch = x[:,:,j:j+H_prime,i:i+W_prime]
    #         image_patch = image_patch.reshape(N,C,H_prime*W_prime)
  
    #         filter = dout.reshape(F,-1)                   
    #                                                                #
    #         dw[:,:,j,i] =  np.dot(filter,image_patch)
    ...
    for j in range(0, HH):
      for i in range(0, WW):
        ...
        image_patch = image_patch.reshape(N, C, H_prime*W_prime)
        # print(np.shape(dout))
        filter = dout.transpose(1, 0, 2, 3). reshape(F, N, H_prime*W_prime)
        # print(np.shape(filter))
        # print(np.shape(image_patch))
        dw[:, :, j, i] = np.tensordot(filter, image_patch, axes=([1, 2], [0, 2]))
        # print(np.shape(dw))
        # k

    # Implementing the dl/dXn,c, and using paddding image performing valid convolution 
    dout_fliiped = dout[:,:,::-1,::-1] #Flipping dout (weight)
    for j in range(0, H):
      for i in range(0, W):
        image_patch = w_pad[:, :, j:j+H_prime, i:i+W_prime]
        image_patch = image_patch.reshape(F, C, H_prime*W_prime)
        # print(np.shape(image_patch))
        filter = dout_fliiped.reshape(N, F, H_prime*W_prime)
        # print(np.shape(filter))
        dx[:, :, j, i] = np.tensordot(filter, image_patch, axes=([1, 2], [0, 2]))
