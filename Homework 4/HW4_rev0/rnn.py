...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        # Forward Pass
        #Step 1
        out1, cache1 = fc_forward(features, W_proj, b_proj)
        #Step 2 
        out2, cache2 = word_embedding_forward(captions_in, W_embed)
        out2_reshape = np.transpose(out2,(1, 0 ,2))
        #Step 3
        out3, cache3 = rnn_forward(out2_reshape, out1, Wx, Wh, b)
        #Step 4
        out3_reshape = np.transpose(out3,(1, 0 ,2))
        hidden_states, cache4 = temporal_fc_forward(out3_reshape,W_vocab, b_vocab)
        #Step 5
        loss, dx = temporal_softmax_loss(hidden_states,captions_out, mask)
        ...
        dx1, dw1, db1 = temporal_fc_backward(dx,cache4)
        grads['W_vocab'] = dw1
        grads['b_vocab'] = db1

        dx1_reshape = np.transpose(dx1,(1, 0 ,2))

        dx2, dh02, dWx2, dWh2, db2 = rnn_backward(dx1_reshape, cache3)
        grads['Wx'] = dWx2
        grads['Wh'] = dWh2
        grads['b'] = db2

        dx2_reshape = np.transpose(dx2,(1, 0 ,2))

        dW_3 = word_embedding_backward(dx2_reshape, cache2)
        grads['W_embed'] = dW_3

        dx4,dWp, dbp  = fc_backward(dh02, cache1)
        grads['W_proj'] = dWp 
        grads['b_proj'] = dbp
    ...
    def sample(self, features, max_length=30):
        ...
        prev_word_idx = [self._start]

        for i in range(max_length):
            prev_word_embed = W_embed[prev_word_idx]
            h, cache_rnn  = rnn_step_forward(prev_word_embed, h, Wx, Wh, b)
            scores, cache_fc = fc_forward(h, W_vocab, b_vocab)
