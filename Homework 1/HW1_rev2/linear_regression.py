...
def compute_objective(X: np.ndarray, y: np.ndarray, w: np.ndarray) -> float:
    ...
    Returns:
        The least square objective term with respect to the coefficient weight w, E(\mathbf{w}).
    """
    objective = 0.0
    ###################################################################
    ...
    # Please check how the objetive is defined from HW1 problem set PDF file.
    # You need to pass the output to the 'objective' variable
    # (i.e., objective = the_final_value_from_your_end )

    # Using closed-form solution
    objective = 0.5 * np.transpose(w) @ np.transpose(X) @ X @ w - np.transpose(w) @ np.transpose(X) @ y + 0.5 * np.dot(y,y)
...
def batch_gradient_descent(
    ...
    *,
    ...
    for current_epoch_number in range(max_epochs):
        ...
        gradient =  np.transpose(X_train) @ X_train @ w - np.transpose(X_train) @ y_train
...
def stochastic_gradient_descent(
    ...
    for current_epoch_number in range(max_epochs):
        for x_data_point, y_data_point in zip(X_train, y_train):
            ...
            gradient = (np.transpose(w) @ x_data_point - y_data_point) * x_data_point
...
def closed_form(
    ...
    # Note/Hint: We have "row-major" version of X_train in this problem.
    # Note that both 2.(c) and 2.(e) call this function, but with different 'reg'.
    ###################################################################
    M = X_train.shape[1]-1
    
    w = np.linalg.inv(reg*np.identity(M+1) + np.transpose(X_train) @ X_train ) @ np.transpose(X_train) @ y_train
    # raise NotImplementedError("TODO: Add your implementation here.")
    ###################################################################
    #                        END OF YOUR CODE                         #
    ###################################################################
...
def compute_rms_for_m(x_train, y_train, x_test, y_test,
    ...
    # Once the train and test data is ready 
    # (in X_train_m and X_test_m, accordingly), please compute w_m.
    ...
    X_train_m = generate_polynomial_features(x_train, M)
    X_test_m = generate_polynomial_features(x_test, M)

    # Regularization factor
    w_m = closed_form(X_train_m,y_train,reg)
...
def closed_form_locally_weighted(
    ...
    Returns:
        The (optimal) coefficient w for the locally weighted linear regression
    ...
    w = np.linalg.inv(np.transpose(X_train) @ R @ X_train ) @ np.transpose(R @ X_train) @ y_train
...
def compute_y_space(
    ...
    for idx, x_point in enumerate(x_space):
        ...
        #########################################################################
        #First: Compute r for x_point
        r_train = np.zeros_like(x_train)
       
        r_train = np.exp(-((2*tau*tau)**-1)*(x_point-x_train)**2)

        # Second: Compute the local w
        w_local = closed_form_locally_weighted(X_train,y_train,r_train)

        # Third: Compute y_space value
        phi_x = generate_polynomial_features(np.array([x_point]), 1)
        
        y_space[idx] = w_local @ np.transpose(phi_x)
